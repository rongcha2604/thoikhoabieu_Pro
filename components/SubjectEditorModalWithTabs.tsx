...
interface SubjectEditorModalWithTabsProps {
...
type TabType = 'official' | 'extra';

const SubjectEditorModalWithTabs: React.FC<SubjectEditorModalWithTabsProps> = ({ 
  subject, 
  onClose, 
  onSave, 
  onDelete, 
  ...
  // Determine initial tab based on subject
  const [activeTab, setActiveTab] = useState<TabType>(
    subject?.isExtraClass ? 'extra' : 'official'
  );
  ...
  const [formData, setFormData] = useState({
    name: subject?.name || '',
    startTime: subject?.startTime || (activeTab === 'extra' ? '18:00' : initialPeriod.startTime),
    endTime: subject?.endTime || (activeTab === 'extra' ? '20:00' : initialPeriod.endTime),
    ...
    notes: subject?.notes || '',
    tags: subject?.tags || [],
    isExtraClass: subject?.isExtraClass ?? (activeTab === 'extra'),
  });
  ...
  useEffect(() => {
    if (subject) {
      setFormData({
        ...
        notes: subject.notes || '',
        tags: subject.tags || [],
        isExtraClass: subject.isExtraClass ?? false,
      });
      setActiveTab(subject.isExtraClass ? 'extra' : 'official');
    } else {
      // Reset form for new subject
      const initialPeriod = classPeriods.morning[0] || CLASS_PERIODS.morning[0];
      setFormData({
        name: '',
        startTime: activeTab === 'extra' ? '18:00' : initialPeriod.startTime,
        endTime: activeTab === 'extra' ? '20:00' : initialPeriod.endTime,
        ...
        notes: '',
        tags: [],
        isExtraClass: activeTab === 'extra',
      });
    }
  }, [subject, initialDay, classPeriods, activeTab]);

  // Reset time when switching tabs
  useEffect(() => {
    if (!subject) {
      const initialPeriod = classPeriods.morning[0] || CLASS_PERIODS.morning[0];
      setFormData(prev => ({
  ...
  }, [activeTab, classPeriods, subject]);
  ...
  const handleSave = () => {
    ...
    if (activeTab === 'extra' && formData.startTime >= formData.endTime) {
      alert(language === 'vi' 
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold">
            {subject 
              ? (language === 'vi' ? 'Sửa môn học' : 'Edit Subject')
              : (language === 'vi' ? 'Nhập môn học' : 'Add Subject')
            }
          </h2>
        ...
        <div className="flex gap-2 mb-6 border-b border-border-light dark:border-border-dark">
          ...
          >
            {language === 'vi' ? '📖 Chính thức' : '📖 Official'}
          </button>
          <button
            type="button"
          ...
          >
            {language === 'vi' ? '📚 Học thêm' : '📚 Extra Class'}
          </button>
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} className="space-y-4">
          ...
          {/* Time Picker - Different for official vs extra */}
          {activeTab === 'official' ? (
            <div>
...
export default SubjectEditorModalWithTabs;
