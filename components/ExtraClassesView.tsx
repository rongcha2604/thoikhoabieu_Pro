...
const ExtraClassesView: React.FC<ExtraClassesViewProps> = ({ subjects, onEditSubject }) => {
  const { language } = useContext(SettingsContext);
  const t = TRANSLATIONS[language];
  const days = DAYS_OF_WEEK[language];

  // Filter chỉ hiển thị các môn học thêm
  const extraClasses = useMemo(() => {
    return subjects.filter(subject => subject.isExtraClass === true);
  }, [subjects]);

  const subjectsByDay = useMemo(() => {
    const grouped: Subject[][] = [[], [], [], [], [], [], []];
    extraClasses.forEach(subject => {
...
export default ExtraClassesView;
